...
object ZIO extends MutatorGroup {
  ...
  private val When: SimpleMutator =
    new SimpleMutator("When") {
      override def getMutator(implicit doc: SemanticDocument): MutationResult = {
        case when @ Term.Apply(Term.Select(zio, Term.Name("when")), paramList)
            if SymbolMatcher
              .exact("zio/ZIO#when().")
              .matches(when.symbol) =>
          default(
            ...
            Term.Apply(Term.Select(zio, Term.Name("as")), List(Term.Name("None"))),
          )
      }
    }
  ...
  private val Unless: SimpleMutator =
    new SimpleMutator("Unless") {
      override def getMutator(implicit doc: SemanticDocument): MutationResult = {
        case when @ Term.Apply(Term.Select(zio, Term.Name("unless")), paramList)
            if SymbolMatcher
